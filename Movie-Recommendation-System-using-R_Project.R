...
setwd("D:/College Work/College Work Assignments/SEM4/Data Science/CP/Movie-Recommendation-System-using-R_Project/IMDB-Dataset")
  ...
  sidebarLayout(
    sidebarPanel(
      selectInput(inputId = "genre", label = "Select Genre", 
                  ...
                  selected = "Action"),
      selectInput(inputId = "type", label = "Select Type", 
                  choices = c("Movie", "TV Show"), selected = "Movie")
    ),
    mainPanel(
      h3("Recommended Movies/TV Shows"),
...
server <- function(input, output) {
  ...
  recommend_movies <- function(genre, type) {
    movies %>%
      filter(genre == genre, type == type) %>%
      arrange(desc(rating)) %>%
      slice(1:5) %>%
  ...
  output$recommendations <- renderPrint({
    recommend_movies(input$genre, input$type)
  })
}
